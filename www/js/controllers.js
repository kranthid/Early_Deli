...
//Main menu conroller
.controller('MainMenuCtrl', function($scope, $stateParams, $timeout, Menu, ionicMaterialInk, ionicMaterialMotion) {
    // Set Header
    $scope.$parent.showHeader();
    $scope.$parent.clearFabs();
    $scope.$parent.setHeaderFab('left');
    //get Menu list
    $scope.menu = Menu.getMainMenu();
    /*Menu.all().then(function(res){
        $scope.menu = res.data
    },function(err){
        console.log("err is >>>",err);
...
.controller('GroceryCtrl', function($scope, $stateParams, $timeout, Menu, Grocery, ngCart, ionicMaterialInk, ionicMaterialMotion) {
    $scope.$parent.showHeader();
    $scope.$parent.clearFabs();
    $scope.isExpanded = true;
    $scope.$parent.setExpanded(true);
    ...
    //
    $scope.menu = Menu.get($stateParams.chatId);
    $scope.food_details = Grocery.all();
    $scope.cartColor = (ngCart.$cart.items.length)?true:false;
    $scope.cartCount = (ngCart.$cart.items.length);
    ngCart.setTaxRate(7.5);
    ngCart.setShipping(2.99);
...
.controller('FruitsCtrl', function($scope, $stateParams, $timeout, Menu, Fruits, ngCart, ionicMaterialInk, ionicMaterialMotion) {
    $scope.$parent.showHeader();
    $scope.$parent.clearFabs();
    $scope.isExpanded = true;
    $scope.$parent.setExpanded(true);
